...
--**--**--**--
  ...
  BEGIN
    ...
    BEGIN
      ...
      SELECT TO_CHAR(TO_DATE(P_FECHA,'DD.MM.YYYY')-1,'DD.MM.YYYY') 
      INTO V_FECHA
      FROM DUAL;
      -- MERGE 1
      ...
      USING (WITH DATOS AS  (SELECT  /*+ INLINE */
                            ...
                            WHERE TO_CHAR(FECHA,'DD.MM.YYYY') = V_FECHA) --SUBSTR(P_FECHA,1,8))
  ...
  PROCEDURE INS_ALC_STATS_CPUMEM_IBHW(P_FECHA_DOMINGO IN VARCHAR2,P_FECHA_SABADO IN VARCHAR2)  AS
    ...
    FROM  (SELECT DOMINGO  FECHA,
                  ...
                  ROW_NUMBER() OVER (PARTITION BY --TRUNC(FECHA),
                                    ...
                                    ORDER BY  SYSTEM_CPU_USAGE DESC) RN
          FROM  ALC_STATS_CPUMEM_HOUR
          WHERE TRUNC(FECHA) BETWEEN TO_DATE(DOMINGO,'DD.MM.YYYY') AND TO_DATE(SABADO,'DD.MM.YYYY') + 86399/86400
        )
    WHERE RN <= limit_in
    GROUP BY  FECHA,
              MONITORED_OBJECT_SITE_ID,
  ...
  BEGIN
    -- STATS_IPRAN
    G_ERROR_LOG_NEW.P_LOG_ERROR('INS_ALC_STATS_IPRAN_DAY',0,'NO ERROR','COMIENZO DE SUMARIZACION DAY ALC_STATS_IPRAN_DAY('||P_FECHA||')');
    INS_ALC_STATS_IPRAN_DAY(P_FECHA);
    G_ERROR_LOG_NEW.P_LOG_ERROR('INS_ALC_STATS_IPRAN_DAY',0,'NO ERROR','FIN DE SUMARIZACION DAY');
    --
    G_ERROR_LOG_NEW.P_LOG_ERROR('INS_ALC_STATS_IPRAN_DAY',0,'NO ERROR','COMIENZO CALCULO BH ALC_STATS_IPRAN_DAY('||P_FECHA||')');
    INS_ALC_STATS_IPRAN_BH(P_FECHA);
    G_ERROR_LOG_NEW.P_LOG_ERROR('INS_ALC_STATS_IPRAN_DAY',0,'NO ERROR','FIN CALCULO BH');
    --
    -- STATS_CPUMEN
    G_ERROR_LOG_NEW.P_LOG_ERROR('INS_ALC_STATS_CPUMEM_DAY',0,'NO ERROR','COMIENZO DE SUMARIZACION DAY ALC_STATS_CPUMEM_DAY('||P_FECHA||')');
    INS_ALC_STATS_CPUMEM_DAY(P_FECHA);
    G_ERROR_LOG_NEW.P_LOG_ERROR('INS_ALC_STATS_CPUMEM_DAY',0,'NO ERROR','FIN DE SUMARIZACION DAY');
    --
    G_ERROR_LOG_NEW.P_LOG_ERROR('INS_ALC_STATS_CPUMEM_BH',0,'NO ERROR','COMIENZO CALCULO BH ALC_STATS_CPUMEM_BH('||P_FECHA||')');
    INS_ALC_STATS_CPUMEM_BH(P_FECHA);
