CREATE OR REPLACE PACKAGE G_ALC_IPRAN AS 
  ...
  */
  ...
  FUNCTION GET_XML_CARDSTATUS_DATA(P_FECHAHORA IN VARCHAR2) RETURN ALC_CARDSLOT_RAW_TY PIPELINED;
  FUNCTION GET_XML_PHYSICALPORT_DATA(P_FECHAHORA IN VARCHAR2) RETURN ALC_PHYSICALPORT_IPRAN_RAW_TY PIPELINED;
  FUNCTION GET_IPRAN_EQUIPOS(P_EXCLUIR_EQUIPO IN VARCHAR2 DEFAULT 'NINGUNO') RETURN CSCO_NODO_INTERFAZ_TAB PIPELINED;
  FUNCTION GET_IPRAN_INTERFACE_POR_EQUIPO(P_EQUIPO IN VARCHAR2) RETURN CSCO_NODO_INTERFAZ_TAB PIPELINED;
  -- PROCEDURES
  PROCEDURE INS_ALC_CARDSLOT_IPRAN_RAW(P_FECHAHORA IN VARCHAR2);
  /**
  ...
  *
  */
  PROCEDURE INS_ALC_PHYSICALPORT_IPRAN_RAW(P_FECHAHORA IN VARCHAR2);
  /**
...
CREATE OR REPLACE PACKAGE BODY G_ALC_IPRAN AS
  --**--**--**--
  FUNCTION GET_IPRAN_EQUIPOS(P_EXCLUIR_EQUIPO IN VARCHAR2 DEFAULT 'NINGUNO') RETURN CSCO_NODO_INTERFAZ_TAB PIPELINED AS
  BEGIN
    EXECUTE IMMEDIATE 'alter session set nls_date_format = ''DD.MM.YYYY HH24:MI:SS''';
    FOR DATO IN (SELECT  MONITORED_OBJECT_SITE_NAME
                FROM     ALC_STATS_IPRAN_HOUR
                WHERE    TO_CHAR(FECHA,'DD.MM.YYYY') = TO_CHAR(SYSDATE-1,'DD.MM.YYYY')
                AND      MONITORED_OBJECT_SITE_NAME NOT IN (P_EXCLUIR_EQUIPO)
                GROUP BY MONITORED_OBJECT_SITE_NAME
                ORDER BY 1 ASC) LOOP
      PIPE ROW(CSCO_NODO_INTERFAZ_OBJ(DATO.MONITORED_OBJECT_SITE_NAME));
    END LOOP;
  END GET_IPRAN_EQUIPOS;
  --**--**--**--
  FUNCTION GET_IPRAN_INTERFACE_POR_EQUIPO(P_EQUIPO IN VARCHAR2) RETURN CSCO_NODO_INTERFAZ_TAB PIPELINED AS
  BEGIN
    EXECUTE IMMEDIATE 'alter session set nls_date_format = ''DD.MM.YYYY HH24:MI:SS''';
    FOR DATO IN (SELECT  SUBSTR(MONITORED_OBJECT_POINTER,
                              INSTR(MONITORED_OBJECT_POINTER,
                              'shelf')+6
                              ,TO_NUMBER(REGEXP_INSTR(MONITORED_OBJECT_POINTER,':',1,3)) - TO_NUMBER(INSTR(MONITORED_OBJECT_POINTER,'shelf',1,1)+6))||'-'||
                       SUBSTR(MONITORED_OBJECT_POINTER,
                              INSTR(MONITORED_OBJECT_POINTER,
                              'cardSlot')+9
                              ,TO_NUMBER(REGEXP_INSTR(MONITORED_OBJECT_POINTER,':',1,4)) - TO_NUMBER(INSTR(MONITORED_OBJECT_POINTER,'cardSlot',1,1)+9))||'-'||
                       SUBSTR(MONITORED_OBJECT_POINTER,
                              INSTR(MONITORED_OBJECT_POINTER,
                              'daughterCardSlot')+17
                              ,TO_NUMBER(REGEXP_INSTR(MONITORED_OBJECT_POINTER,':',1,6)) - TO_NUMBER(INSTR(MONITORED_OBJECT_POINTER,'daughterCardSlot',1,1)+17))||'-'||
                       SUBSTR(MONITORED_OBJECT_POINTER,
                              INSTR(MONITORED_OBJECT_POINTER,
                              'daughterCard:port')+18
                              , LENGTH(MONITORED_OBJECT_POINTER)) INTERFAZ
                FROM ALC_STATS_IPRAN_HOUR
                WHERE TO_CHAR(FECHA,'DD.MM.YYYY') = TO_CHAR(SYSDATE-1,'DD.MM.YYYY')
                AND MONITORED_OBJECT_SITE_NAME = P_EQUIPO
                GROUP BY MONITORED_OBJECT_POINTER
                ORDER BY 1 ASC) LOOP
      PIPE ROW(CSCO_NODO_INTERFAZ_OBJ(DATO.INTERFAZ));
    END LOOP;
  END GET_IPRAN_INTERFACE_POR_EQUIPO;
  --**--**--**--
...
--**--**--**--
  ...
  --**--**--**--
  FUNCTION GET_XML_CARDSTATUS_DATA(P_FECHAHORA IN VARCHAR2) RETURN ALC_CARDSLOT_RAW_TY PIPELINED AS
  BEGIN
    EXECUTE IMMEDIATE 'alter session set nls_date_format = ''DD.MM.YYYY HH24:MI:SS''';
  ...
  --**--**--**--
  PROCEDURE INS_ALC_CARDSLOT_IPRAN_RAW(P_FECHAHORA IN VARCHAR2) AS
    CURSOR DATOS(FECHA_HORA VARCHAR2) IS
    SELECT  FECHA,
            SITE_NAME,
            EQUIPMENT_CATEGORY,
  ...
  BEGIN
    ...
    BEGIN
      ...
      SELECT TO_CHAR(TO_DATE(P_FECHA,'DDMMYYYY')-1,'DDMMYYYY') --TO_CHAR(TO_DATE(P_FECHA,'YYYYMMDD')-1,'YYYYMMDD') 
      INTO V_FECHA
      FROM DUAL;
      -- MERGE 1
      ...
      USING (WITH DATOS AS  (SELECT  /*+ INLINE */
                            ...
                            WHERE TO_CHAR(FECHA,'DDMMYYYYHH24') = V_FECHA||12)
      ...
      EXCEPTION
        WHEN OTHERS THEN
          G_ERROR_LOG_NEW.P_LOG_ERROR('INS_ALC_CARDSLOT_IPRAN_OBJ',SQLCODE,SQLERRM,'MERGE A --> '||P_FECHA||' '||DBMS_UTILITY.format_error_backtrace );
    END;
    -- MERGE 2
    BEGIN
    --
  ...
  --**--**--**--
  FUNCTION GET_XML_PHYSICALPORT_DATA(P_FECHAHORA IN VARCHAR2) RETURN ALC_PHYSICALPORT_IPRAN_RAW_TY PIPELINED AS
  BEGIN
    EXECUTE IMMEDIATE 'alter session set nls_date_format = ''DD.MM.YYYY HH24:MI:SS''';
  ...
  --**--**--**--
  PROCEDURE INS_ALC_PHYSICALPORT_IPRAN_RAW(P_FECHAHORA IN VARCHAR2) AS
    CURSOR DATOS(FECHA_HORA VARCHAR2) IS
    SELECT  FECHA,
            SPECIFIC_TYPE,
            DESCRIPTION,
            ...
            WHEN OTHERS THEN
              ...
              loop
                  ...
                  G_ERROR_LOG_NEW.P_LOG_ERROR('INS_ALC_PHYSICALPORT_IPRAN_RAW',L_ERRNO,L_MSG,
                                              ' FECHA => '||V_ALC_PHYSICALPORT_RAW(I).FECHA||                                                                                                                 
                                              ' SPECIFIC_TYPE => '||V_ALC_PHYSICALPORT_RAW(I).SPECIFIC_TYPE||                                                                                                 
                                              ' DESCRIPTION => '||V_ALC_PHYSICALPORT_RAW(I).DESCRIPTION||                                                                                                     
                                              ' MODO => '||V_ALC_PHYSICALPORT_RAW(I).MODO||                                                                                                                   
                                              ' MAC_ADDRESS => '||V_ALC_PHYSICALPORT_RAW(I).MAC_ADDRESS||                                                                                                     
                                              ' MTU_VALUE => '||V_ALC_PHYSICALPORT_RAW(I).MTU_VALUE||                                                                                                         
                                              ' SPEED => '||V_ALC_PHYSICALPORT_RAW(I).SPEED||                                                                                                                 
                                              ' ACTUAL_SPEED => '||V_ALC_PHYSICALPORT_RAW(I).ACTUAL_SPEED||                                                                                                   
                                              ' NUMBER_OF_POSSIBLE_CHANNELS => '||TO_CHAR(V_ALC_PHYSICALPORT_RAW(I).NUMBER_OF_POSSIBLE_CHANNELS)||                                                            
                                              ' SITE_ID => '||V_ALC_PHYSICALPORT_RAW(I).SITE_ID||                                                                                                             
                                              ' SITE_NAME => '||V_ALC_PHYSICALPORT_RAW(I).SITE_NAME||                                                                                                         
                                              ' OPERATIONAL_STATE => '||V_ALC_PHYSICALPORT_RAW(I).OPERATIONAL_STATE||                                                                                         
                                              ' ADMINISTRATIVE_STATE => '||V_ALC_PHYSICALPORT_RAW(I).ADMINISTRATIVE_STATE||                                                                                   
                                              ' OLC_STATE => '||V_ALC_PHYSICALPORT_RAW(I).OLC_STATE||                                                                                                         
                                              ' OBJECT_FULL_NAME => '||V_ALC_PHYSICALPORT_RAW(I).OBJECT_FULL_NAME);
              end loop;
          -- end --
      END;
      EXIT WHEN DATOS%NOTFOUND;
  ...
  PROCEDURE INS_ALC_PHYSICALPORT_IPRAN_OBJ(P_FECHA IN VARCHAR2) AS
    ...
    BEGIN
      ...
      BEGIN
        ...
        EXCEPTION
        WHEN OTHERS THEN
          G_ERROR_LOG_NEW.P_LOG_ERROR('INS_ALC_PHYSICALPORT_IPRAN_OBJ',SQLCODE,SQLERRM,P_FECHA);
      END;
    ...
    BEGIN
      ...
      USING (WITH DATOS AS  (SELECT  /*+ INLINE */
                            ...
                            WHERE TO_CHAR(FECHA,'DDMMYYYYHH24') = V_FECHA||12) --SUBSTR(P_FECHA,1,8))
            SELECT VALID_START_DATE
                  ,SITE_ID
                  ,SITE_NAME
                  ,MAC_ADDRESS
      ...
      EXCEPTION
        WHEN OTHERS THEN
          G_ERROR_LOG_NEW.P_LOG_ERROR('INS_ALC_PHYSICALPORT_IPRAN_OBJ',SQLCODE,SQLERRM,'MERGE A --> '||P_FECHA||' '||DBMS_UTILITY.format_error_backtrace );
    END;
    BEGIN
  ...
  BEGIN
    EXECUTE IMMEDIATE 'alter session set nls_date_format = ''DD.MM.YYYY HH24:MI:SS''';
    BEGIN
      DELETE FROM ALC_STATS_IPRAN_IBHW WHERE TO_CHAR(FECHA,'DD.MM.YYYY') = P_FECHA_DOMINGO;
      EXCEPTION
        WHEN OTHERS THEN
  ...
  BEGIN
    EXECUTE IMMEDIATE 'alter session set nls_date_format = ''DD.MM.YYYY HH24:MI:SS''';
    BEGIN 
      DELETE FROM ALC_STATS_CPUMEM_IBHW WHERE TO_CHAR(FECHA,'DD.MM.YYYY') = P_FECHA_DOMINGO;
      EXCEPTION
        WHEN OTHERS THEN
